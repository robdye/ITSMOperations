...
resource containerAppsEnv 'Microsoft.App/managedEnvironments@2024-03-01' = {
  name: 'itsm-ops-${environmentName}-cae'
  location: location
  tags: tags
  properties: {
...
resource digitalWorker 'Microsoft.App/containerApps@2024-03-01' = {
  name: 'itsm-ops-${environmentName}-worker'
  location: location
  tags: tags
  identity: {
    type: 'SystemAssigned'
  ...
  properties: {
    ...
    template: {
      containers: [
        {
          name: 'digital-worker'
          image: '${acrLoginServer}/itsm-digital-worker:latest'
          resources: {
            cpu: json('1.0')
            memory: '2Gi'
          }
          env: [
            ...
            { name: 'COSMOS_DATABASE', value: 'itsm-db' }
...
resource mcpServer 'Microsoft.App/containerApps@2024-03-01' = {
  name: 'itsm-ops-${environmentName}-mcp'
  location: location
  tags: tags
  identity: {
    type: 'SystemAssigned'
  ...
  properties: {
    ...
    template: {
      containers: [
        {
          name: 'mcp-server'
          image: '${acrLoginServer}/itsm-mcp-server:latest'
          resources: {
            cpu: json('0.5')
            memory: '1Gi'
          }
          env: [
            ...
            { name: 'SERVER_BASE_URL', value: 'https://itsm-ops-${environmentName}-mcp.${containerAppsEnv.properties.defaultDomain}' }
            { name: 'SNOW_INSTANCE', value: snowInstanceUrl }
