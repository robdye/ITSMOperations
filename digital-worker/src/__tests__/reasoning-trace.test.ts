...
import {
  ...
  logToolCall, logToolResult, logOutcome, logError,
...
describe('reasoning-trace', () => {
  ...
  it('logs intent classification', () => {
    ...
    const trace = logIntent(convId, 'Show incidents', 'incident-manager', 'Incident Manager', 'high', 'matched keyword');
  ...
  it('logs routing decisions', () => {
    const convId = startConversation();
    const trace = logRouting(convId, 'command-center', 'incident-manager', 'Matched incident domain');
    expect(trace.type).toBe('routing');
  });

  it('logs thinking traces', () => {
    const convId = startConversation();
    const trace = logThinking(convId, 'incident-manager', 'Analyzing user request for incident data');
    expect(trace.type).toBe('llm-thinking');
  });

  it('logs tool calls with sanitized args', () => {
    const convId = startConversation();
    const trace = logToolCall(convId, 'incident-manager', 'get_incidents', { 
      state: 'active', 
  ...
  it('logs outcomes with duration', () => {
    const convId = startConversation();
    const trace = logOutcome(convId, 'incident-manager', 'Found 5 incidents', 1500);
