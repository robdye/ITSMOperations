...
const TABLE_NAME = 'ITSMOperationsAudit';
...
export const SENSITIVE_KEYS = /password|secret|token|api_key|credential|authorization|bearer|cookie/i;
...
export function sanitizeParams(params: string): string {
  try {
    ...
    return params.replace(/(password|secret|token|api_key|credential)["']?\s*[:=]\s*["']?[^"',}\s]+/gi, '$1=[REDACTED]');
  }
}
 ...
 * For Table Storage queries, use the Azure portal or custom KQL.
 */
