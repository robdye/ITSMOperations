 ...
 * Classifies ServiceNow records by sensitivity level and enforces
 * label-aware tool access.
 * 
 * Sensitivity levels (aligned with Microsoft Purview MIP labels):
 * - Public: General KB articles, non-sensitive CIs
 * - Internal: Standard incidents, changes, problems
 * - Confidential: PII-containing records, security incidents
 * - Highly Confidential: Security breach data, compliance records
 */
...
const SENSITIVE_FIELDS = [
  ...
  'salary', 'compensation', 'medical', 'health',
];

// Tables with default higher sensitivity
const TABLE_SENSITIVITY: Record<string, SensitivityLabel> = {
  'sys_user': 'confidential',           // User PII
  'sys_user_group': 'internal',
  'incident': 'internal',
  'change_request': 'internal',
  'problem': 'internal',
  'kb_knowledge': 'public',
  'cmdb_ci': 'internal',
  'sn_si_incident': 'highly-confidential',  // Security incidents
  'audit_log': 'highly-confidential',
  'sys_audit': 'highly-confidential',
};

/**
 * Classify a ServiceNow record by sensitivity.
 */
export function classifyRecord(
  ...
  // Security-related content escalates to highly-confidential
  const textContent = JSON.stringify(record).toLowerCase();
  if (/security.breach|data.leak|unauthorized.access|penetration.test/i.test(textContent)) {
    label = 'highly-confidential';
  }

  return {
    recordId: recordId || (record.sys_id as string) || 'unknown',
    table,
...
export function isOperationAllowed(
  ...
  // Public — all operations allowed
  if (classification.sensitivityLabel === 'public') {
    return { allowed: true };
  }

  // Internal — all operations except bulk export
  ...
  // Confidential — read/write allowed, export/bulk blocked
  if (classification.sensitivityLabel === 'confidential') {
    if (operation === 'export' || operation === 'bulk_read') {
      return {
        allowed: false,
        reason: `${operation} blocked: record contains ${classification.piiDetected ? 'PII data' : 'confidential information'}`,
      };
    }
    return { allowed: true };
  }

  // Highly Confidential — read only, everything else blocked
  if (classification.sensitivityLabel === 'highly-confidential') {
    ...
    return {
      allowed: false,
      reason: `${operation} blocked: highly confidential record. Security team approval required.`,
    };
  }
