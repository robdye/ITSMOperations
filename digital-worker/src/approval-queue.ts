// ITSM Operations — Approval Queue with Adaptive Cards
...
export const CLEANUP_AFTER_MS = 24 * 60 * 60 * 1000; // Remove resolved/expired after 24h
...
function cleanupExpired(): void {
  ...
  for (const [id, action] of pendingActions) {
    ...
    // Remove resolved/expired entries after grace period
...
/**
 * Build a Teams Adaptive Card for approval/rejection.
 * Uses Adaptive Card schema v1.4 (supported by Teams).
 */
function buildApprovalCard(action: PendingAction): any {
  ...
  // Build parameter facts
  const paramFacts = Object.entries(action.parameters)
    .filter(([_, v]) => v !== undefined && v !== null)
    .slice(0, 8) // Limit to 8 params for readability
    .map(([key, value]) => ({
      title: key,
