// ITSM Operations — OpenTelemetry Instrumentation
// Standard OTel with GenAI semantic conventions alongside Agent 365 observability.
// Propagates W3C Trace Context across MCP calls and ServiceNow requests.
...
const TRACER_NAME = 'itsm-operations-worker';
const METER_NAME = 'itsm-operations-worker';
...
export function initTelemetry(): void {
  ...
  const resource = resourceFromAttributes({
    [ATTR_SERVICE_NAME]: 'itsm-operations-worker',
...
export function startInferenceSpan(operationName: string, model: string): Span {
  ...
  const span = tracer.startSpan(`gen_ai.${operationName}`, {
...
export function initMetrics(): void {
  ...
  workerInvocationCounter = meter.createCounter('itsm.worker.invocations', {
    description: 'Number of worker invocations',
    unit: '1',
  });

  toolCallCounter = meter.createCounter('itsm.tool.calls', {
    description: 'Number of tool calls',
    unit: '1',
  });

  responseLatencyHistogram = meter.createHistogram('itsm.response.latency', {
