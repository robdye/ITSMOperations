  ...
  server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    switch (name) {
      ...
      case "harvest-resolution": {
        ...
        const inc = await snow.getIncident(incNumber);
        if (!inc) {
          return textResponse(`Incident ${incNumber} was not found.`);
        }
        const result: any = {
          ...
          resolution: inc.close_notes || null,
          resolvedBy: inc.assigned_to?.display_value || inc.assigned_to || null,
          resolvedAt: inc.resolved_at || inc.closed_at || null,
          priority: inc.priority,
          suggestedKeywords: [inc.category, "resolution", "troubleshooting"].filter(Boolean),
        };
        if (includeWorkaround) result.workaround = null;
      ...
      case "generate-shift-handover": {
        ...
        const team = (args?.team as string) || "Operations";
        const [incidents, changes, problems, slas] = await Promise.all([
          snow.getAllIncidents(200),
        ...
        const incidentData = {
          ...
          closed: recentInc.filter((i: any) => ["Resolved", "Closed", "6", "7"].includes(String(i.state))).length,
          escalated: recentInc.filter((i: any) => String(i.priority).startsWith("1")).length,
          items: recentInc.slice(0, 10).map((i: any) => ({ number: i.number, description: i.short_description, priority: i.priority, status: i.state })),
        };
        const changeData = {
          approved: changes.filter((c: any) => ["Scheduled", "-1"].includes(String(c.state))).length,
          rejected: 0,
          implemented: changes.filter((c: any) => ["Implement", "Review", "0", "2"].includes(String(c.state))).length,
          items: changes.slice(0, 5).map((c: any) => ({ number: c.number, description: c.short_description, status: c.state })),
        };
        const problemData = {
          new: problems.filter((p: any) => ["New", "1"].includes(String(p.state))).length,
          updated: problems.filter((p: any) => !["New", "Closed", "1", "4"].includes(String(p.state))).length,
          items: problems.slice(0, 5).map((p: any) => ({ number: p.number, description: p.short_description, status: p.state })),
        };
        const breached = slas.filter((s: any) => s.has_breached === true || s.has_breached === "true");
        const atRisk = slas.filter((s: any) => {
          const pct = parseInt(String(s.percentage || s.business_percentage || "0"), 10);
          return pct > 75 && s.has_breached !== true && s.has_breached !== "true";
        });
        const slaData = { breached: breached.length, atRisk: atRisk.length, met: slas.length - breached.length - atRisk.length };
        const outstandingItems = [
          ...incidents
            .filter((i: any) => ["1", "2"].some((p) => String(i.priority).startsWith(p)) && !["Resolved", "Closed", "6", "7"].includes(String(i.state)))
            .slice(0, 5)
            .map((i: any) => `${i.number}: ${i.short_description}`),
          ...changes.slice(0, 5).map((c: any) => `${c.number}: ${c.short_description}`),
        ];
        const timeline = recentInc.slice(0, 10).map((i: any) => ({
          time: new Date(i.opened_at).toISOString().split("T")[1]?.slice(0, 5) || "00:00",
          event: `${i.number} opened: ${i.short_description}`,
          severity: String(i.priority).startsWith("1") ? "critical" : String(i.priority).startsWith("2") ? "warning" : "info",
        }));
      ...
      case "harvest-knowledge": {
        ...
        const result: any = {
          ...
          rootCause: inc.close_notes || null,
          resolution: inc.close_notes || null,
          workaround: includeWorkaround ? null : undefined,
      ...
      case "show-command-bridge": {
        const [incidents, openCrs, slas] = await Promise.all([
          snow.getIncidents({ limit: 200 }),
          snow.getOpenChangeRequests(100),
          snow.getTaskSLAs({ limit: 100 } as any),
        ]);
        ...
        const currentOnly = (value: number) => [value];
        ...
        const data = {
          ...
          kpis: {
            p1: { value: p1, trend: currentOnly(p1) },
            slaRisk: { value: slaAtRisk + slaBreached, trend: currentOnly(slaAtRisk + slaBreached) },
            approvals: { value: approvalsWaiting, trend: currentOnly(approvalsWaiting) },
          },
          rings,
      ...
      case "show-estate-heatmap": {
        ...
        const incidents = await snow.getIncidents({ limit: 300 });
      ...
      case "show-change-collisions": {
        ...
        const openCrs = await snow.getOpenChangeRequests(200);
        const now = Date.now();
