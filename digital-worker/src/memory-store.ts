...
export interface MemoryEntry {
  ...
  category: string;      // 'conversation' | 'session' | 'pattern' | 'decision'
  content: string;
  workerId?: string;
...
const TABLE_NAME = 'ITSMOperationsMemory';
...
export async function storeMemory(
  ...
  const entry: MemoryEntry = {
    ...
    tier,
    category,
    content: content.substring(0, 32_000), // Table Storage entity limit
    workerId,
...
/**
 * Store a conversation message (short-term memory).
 * Drop-in replacement for conversation-memory.ts addMessage().
 */
 ...
 * Drop-in replacement for conversation-memory.ts getHistory().
 */
...
/**
 * Store a pattern or decision for long-term learning.
 */
export async function storePattern(
  ...
  await storeMemory('system', 'long', category, content, workerId);
}
